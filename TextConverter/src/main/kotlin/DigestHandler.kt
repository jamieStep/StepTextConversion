...
object DigestHandler {
  ...
  fun checkFileDigests ()
  {
    ...
    var msgBits: MutableList<String> = mutableListOf()
  ...
  */
  ...
  {
    val res = getDigests().map { "#  SHA256: ${Paths.get(it.first.toString()).fileName}: ${it.second}" }
  ...
  private fun getDigests (): List<Pair<String, String>>
  {
    ...
    if (null != FileLocations.getInputOsisFilePath())
    {
      m_Input = "OSIS"
      fileList = listOf(FileLocations.getInputOsisFilePath()!!)
    }
    else if (null != FileLocations.getInputVlFilePath())
    {
      m_Input = "VL"
      fileList = listOf(FileLocations.getInputVlFilePath()!!)
    }
    else
    {
      m_Input = "USX"
      fileList = TextStructureUsxForUseWhenAnalysingInput.getFileList(false).map { it.second }
    }

...
object StepStringFormatter
{
  ...
  private fun expandAtReferencesInFormat (theFmt: String): String
  {
    ...
    {
      return ConfigData.get(mr.groups["name"]!!.value) ?: throw StepException("StepStringFormatter couldn't find config attribute " + mr.groups["name"]!!.value)
    }
...
/****************************************************************************/
  ...
  private fun standardiseData (formatString: String, rawArgs: Map<String, Any>): Pair<String, List<Any>>
  {
    ...
    if (n > 0)
    {
      if (hadIdentifiers) throw StepException("Mixed forms of identifier in format string: $formatString")

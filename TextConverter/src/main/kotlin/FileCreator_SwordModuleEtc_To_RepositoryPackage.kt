...
object FileCreator_SwordModuleEtc_To_RepositoryPackage: ProcessingChainElement
 {
  /****************************************************************************/
  /****************************************************************************/
  /**                                                                        **/
  ...
  override fun takesInputFrom() = Pair(FileLocations.getInternalSwordFolderPath(), "zip")
  ...
  private fun doIt()
  {
    ...
    val inputOsis = if (FileLocations.getInputUsxFilesExist()) FileLocations.getInputUsxFolderPath() else null
    val inputUsx = if (null == FileLocations.getInputVlFilePath()) null else FileLocations.getInputVlFolderPath()
    val inputVl  = if (null == FileLocations.getInputVlFilePath()) null else FileLocations.getInputVlFolderPath()

    makeReadMe(inputVl = null != inputVl, inputUsx = null != inputUsx)
  ...
  /****************************************************************************/
  private fun makeReadMe (inputVl: Boolean, inputUsx: Boolean)
  {
    val inputVl  = if (StepFileUtils.isNonEmptyFolder(FileLocations.getInputVlFolderPath()))  "InputVl" else null
    val inputUsx = if (StepFileUtils.isNonEmptyFolder(FileLocations.getInputUsxFolderPath())) "InputUsx" else null
    val alternativeFormat = inputVl ?: inputUsx

    val dateTimeStamp = SimpleDateFormat("yyyy-MM-dd'T'HH:mm").format(Date())

    var originalSource = "Input" + StepStringUtils.sentenceCaseFirstLetter(ConfigData["stepProcessingOriginalData"]!!)

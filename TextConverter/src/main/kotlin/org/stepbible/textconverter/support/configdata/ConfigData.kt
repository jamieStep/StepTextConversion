...
object ConfigData
{
    ...
    */
    ...
    fun put (key: String, theValue: String, force: Boolean)
    {
      ...
      /* If we're forcing, then it's always ok to write to the store because
         ...
         any existing entry was not forced. */
    ...
    private fun getInternal (key: String, nullsOk: Boolean): String?
    {
        ...
        if (null != calculated) return calculated
    ...
    */
    ...
    /**************************************************************************/
     private val C_patGetExternalRevised = Regex("(?i)(?<pre>.*?)@getExternal\\[(?<selector>\\w+)\\W+(?<xpath>.+?)](\\.to(?<fn>\\w+)\\(\\))?(?<post>.*)")
    ...
    */
    ...
    fun makeStepDescription (): String
    {
        ...
        val vernacularTitleIfNotSameAsEnglish = makeStepDescription_getVernacularTitleIfNotSameAsEnglish(englishTitle)
    ...
    */
    ...
    /****************************************************************************/
    /* Want the vernacular name only on non-English Bibles, and even then only if
    not already in the English title. */

    private fun makeStepDescription_getVernacularTitleIfNotSameAsEnglish(theEnglishTitle: String): String
    {
        if ("eng".equals(get("stepLanguageCode3Char"), ignoreCase = true)) return ""

        val stepBibleNameVernacular = get("stepBibleNameVernacular")
        val englishTitle = theEnglishTitle.lowercase()
        val s = stepBibleNameVernacular!!.lowercase()

        if (s.contains(englishTitle) || englishTitle.contains(s)) return ""

        return "($stepBibleNameVernacular) "
    }

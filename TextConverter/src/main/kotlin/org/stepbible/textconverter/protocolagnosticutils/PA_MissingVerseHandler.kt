...
class PA_MissingVerseHandler (fileProtocol: X_FileProtocol)
{
  companion object {
    /**************************************************************************/
    /**
  ...
  *
  * This is called both for verses which were missing in the raw text and for
  ...
  */
  ...
  fun createEmptyVerseForElision (doc: Document, refKey: RefKey, reasonMarker: String = "elision"): List<Node>
  {
    ...
    return listOf(sid, content, m_FileProtocol.makeVerseEidNode(doc, Pair(refKey, null)))
  }
  ...
  */
   ...
   * parameters.
   *
   * @param doc Document which will own the new nodes.
   * @param sidRefKey sid to be used in new verse.
   * @return A list containing the sid, the content and the eid.
   */

  fun createEmptyVerseForReversification (doc: Document, sidRefKey: RefKey): List<Node>
  {
    //Dbg.d("EmptyVerseForReversification: " + Ref.rd(sidRefKey).toString())
    val start = m_FileProtocol.makeVerseSidNode(doc, Pair(sidRefKey, null))
    val end   = m_FileProtocol.makeVerseEidNode(doc, Pair(sidRefKey, null))
    ...
    return listOf(start, content, end)
  }

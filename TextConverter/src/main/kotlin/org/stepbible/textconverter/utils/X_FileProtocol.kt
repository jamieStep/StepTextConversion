...
open class X_FileProtocol
{
  ...
  open fun isAcrosticDivNode (node: Node): Boolean = throw StepExceptionShouldHaveBeenOverridden()
  open fun isAcrosticSpanNode (node: Node): Boolean = throw StepExceptionShouldHaveBeenOverridden()
...
object Osis_FileProtocol: X_FileProtocol()
{
  ...
  */
  ...
  init {
    ...
    m_TagDetails["#document"] = TagDescriptor('N', 'N') // Document.
    ...
    m_TagDetails["cell"] = TagDescriptor('?', 'N') // Chapter is used as syntactic sugar for the div element. It will most often be found in nonbiblical  I turn div type='chapter' into <chapter>, so there is no need to worry about div type='chapter'.
...
object Usx_FileProtocol: X_FileProtocol()
{
  ...
  override fun isAcrosticDivNode (node: Node): Boolean = "para:qa" == Osis_FileProtocol.getExtendedNodeName(node)

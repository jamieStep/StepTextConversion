...
abstract class Z_BibleStructure protected constructor (fileProtocol: Z_FileProtocol?)
{
  /****************************************************************************/
  /****************************************************************************/
  /**                                                                        **/
  ...
  companion object
  {
    ...
    */
    ...
    fun makeOsis2modNrsvxSchemeInstance (bibleStructureBeingWorkedOn: Z_BibleStructure) = makeOsis2modSchemeInstance("NRSV" + (if (bibleStructureBeingWorkedOn.hasAnyBooksDc()) "A" else ""))
    ...
    */
    ...
    fun compareWithGivenScheme (bookNumber: Int, schemeA: Z_BibleStructure, schemeB: Z_BibleStructure): ComparisonWithOtherScheme
    {
  ...
  */
  ...
  protected open fun load (doc: Document, wantWordCount: Boolean)
  {
    ...
    /**************************************************************************/
    val allNodes = Dom.getNodesInTree(doc)
    allNodes.forEach {
 ...
 */
  ...
  private fun makeElts (refAsString: String) = Ref.rdUsx(refAsString).getCopyOfElements()
...
class Osis_BibleStructure: Z_BibleStructure (Osis_FileProtocol)
{
  /****************************************************************************/
  ...
  override fun getRelevanceOfNode (node: Node): NodeRelevance
  {
    when (Dom.getNodeName(node))
    {
      "verse" ->
      {
        val id = node["sID"] ?: node["eID"]!!
        if (RefBase.C_DummyElement == RefCollection.rdOsis(id).getLowAsRef().getV()) // Ignore dummy verses which I insert at the ends of chapters to simplify processing.
      ...
      "chapter" ->
      {
        m_CurrentChapterRefAsString = getStandardisedId(node, "osisID")
      }
...
class Usx_BibleStructure: Z_BibleStructure(Usx_FileProtocol)
{
  /****************************************************************************/
  /* This determines the relevance of any given node to the 'load' function
     in BibleStructure.  That function is interested in verse starts and ends,
...
open class BibleStructureOsis2ModScheme (scheme: String): Z_BibleStructure(null)
{
  /****************************************************************************/

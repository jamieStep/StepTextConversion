...
open class Z_FileProtocol
{
  ...
  open fun getBookCode (doc: Document): String = throw StepExceptionShouldHaveBeenOverridden() // Assumes just one book per file.
  open fun getBookCode (rootNode: Node): String = throw StepExceptionShouldHaveBeenOverridden()
  ...
  open fun isCanonicalHeader (node: Node): Boolean = throw StepExceptionShouldHaveBeenOverridden()
  ...
  open fun makeFootnoteNode (doc: Document, refKey: RefKey, caller: String): Node = throw StepExceptionShouldHaveBeenOverridden()
  open fun makeVerseEid (doc: Document, refKey: RefKey): Node = throw StepExceptionShouldHaveBeenOverridden()
  open fun makeVerseSid (doc: Document, refKey: RefKey): Node = throw StepExceptionShouldHaveBeenOverridden()
...
object Osis_FileProtocol: Z_FileProtocol()
{
  /****************************************************************************/
  ...
  override fun getBookCode (doc: Document) = getBookCode(Dom.findNodeByName(doc,"book", false)!!)
  override fun getBookCode (rootNode: Node) = rootNode["osisID"]!!
  override fun bookNameToNumber (name: String) = BibleBookNamesOsis.nameToNumber(name)

  override fun isCanonicalHeader (node: Node) = "title:psalm" == getExtendedNodeName(node)
  ...
  * @param refKey Reference with which the footnote is associated.
  * @param caller Callout for footnote.
  * @return Footnote node.
  */

  override fun makeFootnoteNode (doc: Document, refKey: RefKey, caller: String): Node
  {
    val id = Ref.rd(refKey).toStringOsis()
    return Dom.createNode(doc, "<note type='explanation' osisRef='$id' osisID='$id!${Globals.getUniqueIdCounter()}' n='$caller'/>")
  }
  ...
  */
  ...
  override fun makeVerseEid (doc: Document, refKey: RefKey): Node
  {
  ...
  */
  ...
  override fun makeVerseSid (doc: Document, refKey: RefKey): Node
  {
    val refAsString = Ref.rd(refKey).toStringOsis()
  ...
  */
  ...
  init {
    ...
    m_OsisTagDetails["relation"] = OsisTagDescriptor('N', 'N', 'N') // The relation element occurs only in a work element. It has only text only content and is
    ...
    m_OsisTagDetails["salute"] = OsisTagDescriptor('Y', 'N', 'N') // The salute element is used to mark a saluation or opening comments. It is most generally
...
object Usx_FileProtocol: Z_FileProtocol()
{
  /****************************************************************************/
  ...
  override fun getBookCode (doc: Document) = getBookCode(Dom.findNodeByName(doc,"book", false)!!)
  override fun getBookCode (rootNode: Node) = rootNode["code"]!!
  override fun bookNameToNumber (name: String) = BibleBookNamesUsx.nameToNumber(name)

  override fun isCanonicalHeader (node: Node) = "para:d" == Osis_FileProtocol.getExtendedNodeName(node)
  ...
  override fun isInherentlyCanonicalTag (node: Node) = "char:w" == Usx_FileProtocol.getExtendedNodeName(node)

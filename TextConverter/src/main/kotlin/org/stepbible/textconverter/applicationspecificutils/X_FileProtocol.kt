...
/******************************************************************************/
/**
 * In order to promote commonality, it is convenient to hide the differences
 * between the USX and OSIS protocols as far as possible.
 *
 * This class does that.  I have to admit, though, that there has proved to
 * be vastly more of it than I anticipated, and I do wonder to an extent
 * whether this exercise was, in fact, ill-judged.
 *
 * Note that the OSIS implementation, in particular, is incomplete, because
 * there are some things which the OSIS processing -- at least at present --
 * does not require.
 *
...
open class X_FileProtocol
{
  ...
  open fun readRefCollection (text: String): RefCollection = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  ...
  open fun getBookNumber (doc: Document): Int = throw StepExceptionWithStackTraceShouldHaveBeenOverridden() // Assumes just one book per file.
  open fun getBookNumber (rootNode: Node): Int = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  ...
  open fun insertBlankLineIntoTable (tableNode: Node): Unit = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  open fun isAcrosticDivNode (node: Node): Boolean = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  ...
  open fun isNumberedLevelTag (node: Node): Boolean = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  open fun isParaWhichCouldContainMultipleVerses (node: Node): Boolean = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  open fun isPlainVanillaPara (node: Node): Boolean = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  open fun isPoetryPara (node: Node): Boolean = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  open fun isSpanType (node: Node): Boolean = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  open fun isSpeakerNode (node: Node): Boolean = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  open fun isStrongsNode (node: Node): Boolean = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  open fun isTitleNode (node: Node): Boolean = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
  ...
  open fun makeVerseEidNode (doc: Document, refAsString: String): Node = throw StepExceptionWithStackTraceShouldHaveBeenOverridden()
...
Dbg.dCont(key + " : " + Dom.getNodeName(node), "speaker")
  ...
  /****************************************************************************/
  /**
   * Like getVerseEndInteraction, qv, except that it works up the parent chain
...
object Osis_FileProtocol: X_FileProtocol()
{
  ...
  */
  ...
  /****************************************************************************/
  /**
  * Returns an indication of whether the node is of the type implied by the
  * name of the method.
  *
  * @param node
  * @return True if the node is of the type implied by the name of the method.
  */

  override fun isAcrosticDivNode (node: Node): Boolean = "title:acrostic" == getExtendedNodeName(node)
  ...
  */
  ...
  override fun isAcrosticSpanNode (node: Node): Boolean = "h:acrostic" == getExtendedNodeName(node)


  /****************************************************************************/
  /**
  * Returns an indication of whether the given node is a canonical title.
  *
  ...
  */
  ...
  override fun isParaWhichCouldContainMultipleVerses (node: Node) = "p" == Dom.getNodeName(node)
  ...
  *
  * @param node
  * @return True if the node should be treated as canonical.
  */
  ...
  */
  ...
  override fun updateVerseSid (verse: Node, refKeyLow: RefKey, refKeyHigh: RefKey)
  {
    ...
    verse["sID"] = "$refLowAsString-$refHighAsString"
  }
  ...
  */
  ...
  init {
    ...
    m_TagDetails["relation"] = TagDescriptor('X', 'N') // The relation element occurs only in a work element. It has only text content and is
    ...
    m_TagDetails["salute"] = TagDescriptor('?', 'N') // The salute element is used to mark a salutation or opening comments. It is most generally
...
object Usx_FileProtocol: X_FileProtocol()
{
  ...
  override fun getBookNodes (doc: Document) = doc.findNodesByName("book")
  override fun isAcrosticDivNode (node: Node): Boolean = "para:qa" == getExtendedNodeName(node)
  override fun isAcrosticSpanNode (node: Node): Boolean = "char:qac" == getExtendedNodeName(node)
  ...
  */
  ...
  override fun makeVerseEidNode (doc: Document, refKey: Pair<RefKey, RefKey?>): Node
  {
    val refAsString = if (null == refKey.second) Ref.rd(refKey.first).toStringUsx() else RefCollection.rd(listOf(refKey.first, refKey.second!!)).toStringUsx()
    return makeVerseEidNode(doc, refAsString)
  }
  ...
  */
  ...
  /****************************************************************************/
  /**
  * Updates a verse sid tag as a range.
  *
  * @param verse Verse node to be updated.
  * @param refKeyLow Start of range.
  * @param refKeyHigh End of range.
  ...
  */
  ...
  init {
    ...
    m_TagDetails["#document"] = TagDescriptor('N', 'N') // Text.

 ...
 *
 * We still use the Crosswire version when creating public-facing modules
 * because that's the only version which can generate modules usable by
 * standard Crosswire software.  But when handling modules to be used within
 * STEP, we use our own version (which in turns mean that we have to deploy
 * our own version of the Sword library code).
 *
 * Our version requires some supporting data which the Crosswire version
 * does not need.  This class handles provides a uniform interface which
 * hides the differences between the two from the system at large.
 *
...
object Osis2ModInterfaceStep: Osis_Osis2modInterface()
{
  ...
  /****************************************************************************/
  override fun initialise () = initialiseBookDetails()


  /****************************************************************************/
  override fun supportsSubverses () = true

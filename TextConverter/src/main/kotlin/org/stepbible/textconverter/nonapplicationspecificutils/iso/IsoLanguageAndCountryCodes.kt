 ...
 * the other ('terminological') is newer.  According to various non-official
...
object IsoLanguageAndCountryCodes: ObjectInterface
{
  ...
  init
  {
    ...
    FileLocations.getInputStream(FileLocations.getCountryCodeInfoFileName()).first!!.bufferedReader().lines().forEach {
      val line = it.trim()
    ...
    FileLocations.getInputStream(FileLocations.getIsoLanguageCodesFileName()).first!!.bufferedReader().lines().forEach {
      val line = it.trim()
      if (line.isEmpty() || line.startsWith("#!"))
        return@forEach

...
open class SE_CrossBoundaryMarkup protected constructor (fileProtocol: Z_FileProtocol) : SE
{
  /****************************************************************************/
  /****************************************************************************/
  /**                                                                        **/
  ...
  /* Plain vanilla paras occur a lot in most texts, and have a habit of running
     ...
     haven't done so earlier in the processing. */

  protected open fun changeParaPToMilestone (rootNode: Node) { throw StepExceptionShouldHaveBeenOverridden() }
  ...
  {
    Dbg.reportProgress("Handling cross-boundary markup for ${m_FileProtocol.getBookCode(rootNode)}.")
  ...
  private fun deleteVerseEnds (rootNode: Node)
  {
    Dom.findNodesByName(rootNode, m_FileProtocol.tagName_chapter(), false).forEach { chapterNode ->
      val eids = Dom.findNodesByAttributeName(chapterNode, m_FileProtocol.tagName_verse(), m_FileProtocol.attrName_verseEid())
      eids.subList(0, eids.size - 1).forEach(Dom::deleteNode) // Sublist so we retain the verse end for the dummy verse.
    }
  }
  ...
  private fun insertVerseEnd (id: String, sidWhoseEidWeAreCreating: Node, nextVerseSid: Node)
  {
    ...
    {
      Utils.addTemporaryAttribute(a, "_temp_hasCrossBoundaryMarkup", "y")
      Utils.addTemporaryAttribute(b, "_temp_hasCrossBoundaryMarkup", "y")
    }
...
/******************************************************************************/
object Osis_SE_CrossBoundaryMarkup: SE_CrossBoundaryMarkup(Osis_FileProtocol)
{
  override fun changeParaPToMilestone (rootNode: Node)
  {
    val paras = Dom.findNodesByName(rootNode, "p", false)
...
/******************************************************************************/
object Usx_SE_CrossBoundaryMarkup: SE_CrossBoundaryMarkup(Usx_FileProtocol)
{
  override fun changeParaPToMilestone (rootNode: Node)
  {
    val paras = Dom.findNodesByAttributeValue(rootNode, "para", "style", "p")
    paras.forEach { Dom.convertToSelfClosingNode(it) }
    if (paras.isNotEmpty())
      IssueAndInformationRecorder.setForcedSelfClosingParas()
  }

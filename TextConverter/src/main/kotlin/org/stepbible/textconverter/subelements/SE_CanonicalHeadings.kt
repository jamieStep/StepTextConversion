...
open class SE_CanonicalHeadings protected constructor (fileProtocol: Z_FileProtocol): SE
{
  /****************************************************************************/
  /****************************************************************************/
  /**                                                                        **/
  ...
  /****************************************************************************/
  protected open fun getCanonicalTitleNodes (rootNode: Node): List<Node> = throw StepExceptionShouldHaveBeenOverridden()
  protected open fun isCanonicalTitleNode (node: Node): Boolean = throw StepExceptionShouldHaveBeenOverridden()
  protected open fun makeItalicsText (): String = throw StepExceptionShouldHaveBeenOverridden()
  protected open fun makePlainVanillaParaText (): String = throw StepExceptionShouldHaveBeenOverridden()
  ...
  {
    Dbg.reportProgress("Handling canonical headings for ${m_FileProtocol.getBookCode(rootNode)}.")
  ...
  private fun processVersesContainingCanonicalTitles (rootNode: Node)
  {
    ...
    fun processChapter (chapter: Node)
    {
      ...
      for (i in allNodes.indices)
      {
        ...
        else if (isCanonicalTitleNode(node))
        {
          if (-1 != sidIx)
  ...
  private fun amendCanonicalTitle (titleNode: Node, paraBeforeOrAfter: Char)
  {
     ...
     children.filter { val x = Dom.getNodeName(it); x != m_FileProtocol.tagName_verse() && x != m_FileProtocol.tagName_note()}
             .forEach {
               val italicsNode = Dom.createNode(document, makeItalicsText())
     ...
     val para = Dom.createNode(document, makePlainVanillaParaText())
...
/******************************************************************************/
object Osis_SE_CanonicalHeadings: SE_CanonicalHeadings(Osis_FileProtocol)
{
  override fun getCanonicalTitleNodes (rootNode: Node) = Dom.findNodesByAttributeValue(rootNode, "title", "type", "psalm")
  override fun isCanonicalTitleNode (node: Node) = "title" == Dom.getNodeName(node) && "title" == node["type"]
  override fun makeItalicsText () = "<hi type='italic'/>"
  override fun makePlainVanillaParaText () = "<p/>"
}
...
/******************************************************************************/
object Usx_SE_CanonicalHeadings: SE_CanonicalHeadings(Usx_FileProtocol)
{
  override fun getCanonicalTitleNodes (rootNode: Node) = Dom.findNodesByAttributeValue(rootNode, "para", "style", "d")
  override fun isCanonicalTitleNode (node: Node) = "para" == Dom.getNodeName(node) && "d" == node["style"]
  override fun makeItalicsText () = "<char style='it'/>"
  override fun makePlainVanillaParaText () = "<para style='p'/>"
}

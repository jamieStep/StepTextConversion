...
open class SE_RuntimeReversification protected constructor (fileProtocol: Z_FileProtocol): SE
{
  /****************************************************************************/
  /****************************************************************************/
  /**                                                                        **/
  ...
  /****************************************************************************/
  protected open fun getBookName (rootNode: Node): String = throw StepExceptionShouldHaveBeenOverridden()
  protected open fun makeFootnote (rootNode: Node, refKey: RefKey, text: String, caller: String): Node = throw StepExceptionShouldHaveBeenOverridden()
  ...
  */
  ...
  override fun process (rootNode: Node)
  {
    ...
    {
      Dbg.reportProgress("Applying reversification footnotes for runtime reversification for ${m_FileProtocol.getBookCode(rootNode)}.")
  ...
  private fun addFootnote (sidNode: Node, row: ReversificationDataRow)
  {
    ...
    val noteNode = makeFootnote(sidNode, row.standardRefAsRefKey, footnoteText, callout)
  ...
  {
    val bookName = getBookName(rootNode)
    initialise()
    var res = false
...
/******************************************************************************/
object Osis_SE_RuntimeReversification: SE_RuntimeReversification(Osis_FileProtocol)
{
  override fun getBookName (rootNode: Node) = rootNode["osisID"]!!
  override fun makeFootnote (rootNode: Node, refKey: RefKey, text: String, caller: String) = Osis_Utils.makeFootnote(rootNode.ownerDocument, refKey, text, caller)
}
...
/******************************************************************************/
object Usx_SE_RuntimeReversification: SE_RuntimeReversification(Usx_FileProtocol)
{
  override fun getBookName (rootNode: Node) = rootNode!!["code"]!!
  override fun makeFootnote (rootNode: Node, refKey: RefKey, text: String, caller: String) = Usx_Utils.makeFootnote(rootNode.ownerDocument, refKey, text, caller)
}